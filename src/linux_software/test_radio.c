...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    unsigned int nread = 114688;
    start_time = *(periph_base+RADIO_TUNER_TIMER_REG_OFFSET);
    for (int i=0;i<nread;i++)
        stop_time = *(periph_base+RADIO_TUNER_TIMER_REG_OFFSET);

    unsigned int bytes_transferred = nread*4; // 32 bits per word = 4 bytes
    unsigned int time_spent_clk = stop_time-start_time;
...
int main()
{
    ...
    printf("\r\n\r\n\r\nLab 10 Nolan Andreassen - Custom Peripheral Demonstration\n\r");
